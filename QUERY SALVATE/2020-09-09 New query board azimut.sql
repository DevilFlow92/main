...
SELECT 6 categoria, 6.47 prezzo ,250 CodServizio, 'Gestione Az Libera Impresa' DesServizio, 181 codKPI, 'CESAM Numero pratiche lavorate AZ Libera Impresa' desKPI union all --Azimut Libera Impresa
...
SELECT (SELECT CASE WHEN Anno < 2020 then 270000 ELSE 250000 END --2 punti base sul gestito, si aggira sui 16 miliardi circa
...
,az_life AS (
SELECT(SELECT  7500   --servizio canone azlife
...
SELECT(SELECT CASE WHEN rs.S_Data.Data < '20200401' THEN
						18000   --servizio concierge sospesi
		ELSE 5200 END --Servizio Concierge - Distacchi
/ COUNT(ChiaveData) FROM rs.S_Data
WHERE Mese = DATEPART(MONTH,GETDATE()) AND Anno = DATEPART(YEAR,GETDATE()) AND FlagFestivo = 0) canone , DATEPART(MONTH,GETDATE()) mm
FROM rs.S_Data where ChiaveData <= CAST(GETDATE() as DATE) AND ChiaveData >=  DATEADD(month, DATEDIFF(month, 0, GETDATE()), 0) AND FlagFestivo = 0
)
...
+ CASE WHEN anno = 2020 AND Mese IN (1,2,3,4,5,6) THEN 11666 ELSE 0 END --Revenues spot 70k I semestre 2020
/*
...
FROM rs.v_CESAM_AZ_FatturazioneIncarichi_Board v
...
FROM rs.S_Data
WHERE DataInizioSettimana <= CAST(GETDATE() AS DATE)
AND DataInizioSettimana >= '20200101'
