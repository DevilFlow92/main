...
--/****** Object:  View [rpa].[v_RPA_CESAM_CB_CodaLavorazione_BOT_DocBank]    Script Date: 28/12/2017 12:23:48 ******/
--SET ANSI_NULLS ON
--GO
...
--ALTER VIEW [rpa].[v_RPA_CESAM_CB_CodaLavorazione_BOT_DocBank]
...
FROM T_Incarico 
JOIN T_DatoAggiuntivo ON T_DatoAggiuntivo.IdIncarico = T_Incarico.IdIncarico AND CodTipoDatoAggiuntivo = 643 and FlagAttivo = 1
JOIN T_R_Incarico_Persona on T_R_Incarico_Persona.IdIncarico = T_Incarico.IdIncarico 
JOIN T_Persona ON T_Persona.IdPersona  = T_R_Incarico_Persona.IdPersona and Progressivo = 1
JOIN T_Documento on T_Incarico.IdIncarico = T_Documento.IdIncarico
JOIN D_Documento ON T_Documento.Tipo_Documento = D_Documento.Codice AND  CodOggettoControlli = 44 --Che Banca - Apertura CC
...
LEFT JOIN (SELECT max(IdTransizione) idtransizione,max(DataTransizione) DataTransizione,L_WorkflowIncarico.IdIncarico FROM T_Incarico
		JOIN L_WorkflowIncarico ON L_WorkflowIncarico.IdIncarico = T_Incarico.IdIncarico
		WHERE CodTipoIncarico = 331 AND CodArea = 8 AND CodCliente = 48
		AND CodAttributoIncaricoDestinazione = 1457
		...
		) MaxKoLoop ON MaxKoLoop.IdIncarico = T_Incarico.IdIncarico

LEFT JOIN (SELECT MAX(IdTransizione)idtransizione, MAX(DataTransizione)datatransizione, IdIncarico 
		...
		GROUP BY IdIncarico) transizione ON transizione.IdIncarico = T_Incarico.IdIncarico


LEFT JOIN S_Data ON CONVERT(varchar(10),GETDATE(),112) = ChiaveData


WHERE
T_Incarico.IdIncarico = 9494837
